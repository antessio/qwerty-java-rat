...
//				return null;
	...
	public void add(Request request, Response response) throws BufferException {
		if(request.getProcessType()==null || request.getProcessType().getType()!=ProcessType.COMMANDS)
			throw new BufferException("request.processType.not.consistent");
		if(response.getProcessType()==null || response.getProcessType().getType()!=ProcessType.COMMANDS)
