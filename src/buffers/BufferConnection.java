...
public class BufferConnection implements BufferInterface{
	...
	public void add(Request request) throws BufferException{
		if(request.getProcessType()==null || request.getProcessType().getType()!=ProcessType.CONNECTION)
...
//				return null;
	...
	public void add(Request request, Response response) throws BufferException {
		if(request.getProcessType()==null || request.getProcessType().getType()!=ProcessType.CONNECTION)
			throw new BufferException("request.processType.not.consistent");
		if(response.getProcessType()==null || response.getProcessType().getType()!=ProcessType.CONNECTION)
